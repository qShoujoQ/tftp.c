...
uint16_t __a_copy_options_from(tftpc_option_t **dst, const uint8_t *src, uint16_t *out_ocount, uint16_t idx, uint16_t bytes_size) {
    ...
    *dst = malloc ((ocount + 1) * sizeof(tftpc_option_t));
    ...
    while (idx < bytes_size) {
        if (ocount) *dst = realloc(*dst, (ocount + 1) * sizeof(tftpc_option_t));
...
const char *tftpc_opcode_to_string(tftpc_opcode_t opcode) {
	switch (opcode) {
	case TFTP_RRQ: 	 	return "RRQ";
	case TFTP_WRQ: 		return "WRQ";
	case TFTP_DATA: 	return "DATA";
	case TFTP_ACK: 		return "ACK";
	case TFTP_ERROR: 	return "ERROR";
	case TFTP_OACK: 	return "OACK";
...
const char *tftpc_error_to_string(tftpc_error_kind_t kind, uint8_t error) {
	if (kind == ERROR_KIND_LIB) {
		switch (error) {
		case TFTPC_SUCCESS: 				 return "Success";
    	case TFTPC_INVALID_OPCODE: 			 return "Invalid opcode - got packet with undefined opcode, or unexpected packet type";
    	case TFTPC_INVALID_ARGUMENT: 		 return "Invalid argument - received NULL as argument, or argument had unexpected value";
    	case TFTPC_BUFFER_OFFSET_ERROR: 	 return "Buffer offset error - packet is malformed or bug in deserialization code";
    	case TFTPC_MEMORY_ERROR: 			 return "Memory error - malloc, realloc, memcpy, etc. failed";
    	case TFTPC_OPTION_NOT_FOUND: 		 return "Option not found - option not found in packet";
        case TFTPC_TFTP_ERROR:               return "TFTP error - got error packet from server";
        case TFTPC_UNEXPECTED_RESULT:        return "Unexpected result - packet had unexpected contents (lack of options, data, etc.)";
    	default:							 return "Unknown error";
		}
	} else if (kind == ERROR_KIND_TFTP) {
		switch (error) {
		case TFTP_ERROR_UNDEFINED: 			 return "Undefined error, see error message (if any)";
    	case TFTP_ERROR_FILE_NOT_FOUND: 	 return "File not found";
    	case TFTP_ERROR_ACCESS_VIOLATION: 	 return "Access violation";
    	case TFTP_ERROR_DISK_FULL: 			 return "Disk full or allocation exceeded";
    	case TFTP_ERROR_ILLEGAL_OPERATION: 	 return "Illegal TFTP operation";
    	case TFTP_ERROR_UNKNOWN_TRANSFER_ID: return "Unknown transfer ID";
    	case TFTP_ERROR_FILE_ALREADY_EXISTS: return "File already exists";
    	case TFTP_ERROR_NO_SUCH_USER: 		 return "No such user";
    	default: 							 return "Invalid error code";
    	}
	} else if (kind == ERROR_KIND_NET) return "Networking error";
...
tftpc_error_lib_t tftpc_packet_add_option(tftpc_packet_t *packet, const char *name, const char *value) {
	if (packet->opcode != TFTP_RRQ && packet->opcode != TFTP_WRQ && packet->opcode != TFTP_OACK) return TFTPC_INVALID_OPCODE;
	if (!name || !value || !packet) return TFTPC_INVALID_ARGUMENT;
	...
	*ocount += 1;
	...
	if (packet->opcode == TFTP_RRQ || packet->opcode == TFTP_WRQ) packet->contents.RWRQ_T.options = options;
	else packet->contents.OACK_T.options = options;
...
tftpc_packet_t *tftpc_packet_create_request(tftpc_opcode_t packet_kind, const char *file_name, const char *mode) {
	if (packet_kind != TFTP_WRQ && packet_kind != TFTP_RRQ) return NULL;
...
tftpc_packet_t *tftpc_packet_from_bytes(const uint8_t *bytes, uint16_t bytes_size, tftpc_error_lib_t *out_error) {
	...
	if (idx != bytes_size) {
        if (bytes_size - idx == 1 && bytes[idx++] == 0) goto null_byte_appended;
